...
function Log:set_level(level)
  local logger_ok, _ = xpcall(function()
    ...
    if structlog then
      local logger = structlog.get_logger "userconf"
  ...
  local packer_ok, _ = xpcall(function()
    ...
    require("packer.log").new { level = userconf.log.level }
  end, debug.traceback)
...
function Log:init()
  ...
  local log_level = Log.levels[(userconf.log.level):upper() or "WARN"]
  local userconf_log = {
    userconf = {
      sinks = {
  ...
  structlog.configure(userconf_log)
  local logger = structlog.get_logger "userconf"

  -- Overwrite `vim.notify` to use the logger
  if userconf.log.override_notify then
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "userconf")
end
