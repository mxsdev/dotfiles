...
---@param branch string
function M.switch_branch(branch)
...
function M.get_version()
  local current_branch = M.get_branch()

  local lvim_version
  if current_branch ~= "HEAD" or "" then
    lvim_version = current_branch .. "-" .. M.get_current_sha()
  else
    lvim_version = "v" .. M.get_tag()
  end
  return lvim_version
end
