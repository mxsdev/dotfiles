...
function M.config()
  local lvim_dashboard = require "user.core.alpha.dashboard"
  local lvim_startify = require "user.core.alpha.startify"
  userconf.builtin.alpha = {
...
local function resolve_config(theme_name)
  ...
  local section = userconf.builtin.alpha[theme_name].section
...
local function configure_additional_autocmds()
  ...
  if not userconf.builtin.lualine.options.globalstatus then
...
function M.setup()
  ...
  if not alpha_ok then
    Log:warn("Alpha not installed!")
    return
  end

  local mode = userconf.builtin.alpha.mode
  local config = userconf.builtin.alpha[mode].config
