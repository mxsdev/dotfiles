...
local function resolve_config(server_name, ...)
  local common_on_attach = require("user.lsp").common_on_attach
  ...
  local old_attach = defaults.on_attach

  defaults.on_attach = function(...)
    pcall(common_on_attach, ...)
    pcall(old_attach, ...)
  end
...
function M.setup(server_name, user_config)
  ...
  if not server:is_installed() then
    if userconf.lsp.automatic_servers_installation then
